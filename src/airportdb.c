		...
		if (strstr(line, "100 ") == line) {
			...
			if (strcmp(comps[1], "transition_alt") == 0) {
				...
				if (is_valid_lon(lon)) {
					...
					dbg_log(tile, 1, "%s:%d: skipping "
					    "malformed datum_lon line.",
					    apt_dat_fname, line_num);
				}
			}
    ...
    int line_num, const char *line)
{
	...
	if (!is_valid_lat(arpt->refpt.lat)) {
		dbg_log(tile, 1, "Removing airport %s: lat %f invalid.",
    ...
    int line_num, const char *line)
{
	...
	if (!is_valid_elev(telev) ||
		...
		dbg_log(tile, 1, "%s:%d: malformed runway entry, skipping. "
		    "Runway threshold elevation, GPA or TCH are invalid. "
		    "Offending line:\n%s", filename, line_num, line);
		goto out;
	}
