...
static const char *monitor_tooltips[][NUM_MONITORS] = {
	...
	{	/* DIST_RMNG_RTO_MON */
		"ON: distance remaining callouts on rejected takeoff are "
		"enabled.",
		"OFF: distance remaining callouts on rejected takeoff are "
		"disabled.",
		MAN_REF("4.7"),
		NULL
	},
	...
	{	/* APCH_UNSTABLE_MON */
		...
		"checked depend",
		"    on the lower gate setting of the respective approach "
		"monitor.",
...
static const char *speak_units_tooltip[] = {
	"ON: append the units of measure used to initial distance "
	"annunciations.",
...
static const char *nd_alert_overlay_force_tooltip[] = {
	"ON: always display visual alerts using the fallback on-screen "
	"overlay.",
	"OFF: only display visual alerts using the on-screen overlay if the",
	"    aircraft doesn't provide native display of visual alerts on the "
	"ND.",
	MAN_REF("3.1.2"),
	NULL
};
...
static const char *reset_glob_tooltip[] = {
	...
	"associated",
	"aircraft is loaded. Otherwise the default configuration will be "
	"applied.",
	MAN_REF("5"),
	NULL
};
