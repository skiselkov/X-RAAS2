...
\noindent After installation, the first time you start up X-Plane, X-RAAS
will scan all install scenery and extract runway information to build its
own data cache. This can take up to 10 seconds or more (depending on how
much scenery you have). Once the cache is built, X-RAAS will use the
cache and startup will be much faster. The purpose of this scanning
process is to make sure that X-RAAS's runway information matches your
scenery as closely as possible and to avoid generating spurious
annunciations. Once started up, X-RAAS should not impose any significant
additional load on your simulator.

X-RAAS checks for updates to airport scenery or the AIRAC database in the
simulator during startup. If scenery is added or removed, or the AIRAC
cycle is changed, X-RAAS will automatically recreate its airport data
cache.

\notebox{Updates to existing scenery might not be detected, as X-RAAS doesn't
attempt to detect file modifications. If you have significantly updated
existing scenery and would like to force X-RAAS to refresh its airport
data cache, simply use the menu entry ``Plugins'' $\rightarrow$
``X-RAAS'' $\rightarrow$ ``Recreate data cache''.}
...
airliners. To avoid this, by default X-RAAS checks if the aircraft
currently loaded in the simulator meets all of the following criteria:
...
\item Style of runway number pronunciation (should single-digit runways
have a `0' prepended or not).

\item Units of measure (feet or meters).

\item Whether to append units of measure to the initial callout.

\end{itemize}

\noindent If the current simulator view is external, annunciations are
suppressed.
...
requires 3rd party aircraft integration. If an aircraft does not provide
this integration, X-RAAS will by default fall back to display visual
annunciations using the on-screen overlay. If the current simulator view
is external, annunciations are suppressed.
...
This section lists all the various normal and caution advisories X-RAAS
can issue for various potential hazards. It is organized by phase of
flight, starting with initially approaching a runway on the ground for
takeoff and progressing towards a landing and runway exit.
...
displaced end. Stopways are not placed in the bounding box.}. X-RAAS will
issue an advisory when the aircraft's nose is approximately 1 second from
penetrating this bounding box (calculated based on ground speed). The
advisory names the runway end closest to the aircraft.
...
when ground speed exceeds 40 knots to prevent activation on takeoff
through intersecting runways. Please also note that the annunciation does
not guarantee the ability to stop before entering the runway.
...
amber `FLAPS' visual caution advisory will be issued instead of the green
`ON XX' advisory:
...
RUNWAY 09 RIGHT, ON RUNWAY 09 RIGHT'') and displays an amber `ON XX'
visual advisory. The default intervals are defined as follows:
...
\item Maximum number of repetitions: 3

\end{itemize}

\noindent After the advisory has been repeated for the maximum number of
repetitions, further advisories are inhibited until the aircraft lines up
with another runway. Refer to section \ref{sec:Configuration} for the
interval configuration parameters \confopt{on\_rwy\_warn\_initial},
...
minimum for a safe takeoff, the ``ON RUNWAY'' annunciation is supplemented
by a runway distance available readout (rounded down to the nearest 100
feet or meters). The aural advisory is accompanied by a non-routine amber
visual advisory on the ND:

\visualadvisory{nonroutine}{ON XX YY}

\noindent Where `XX' is the runway identifier and `YY' is the runway
length available in hundreds of feet or meters.
...
operator defined minimum, once ground speed exceeds 40 knots, a warning
...
\item Aircraft is NOT on a runway
...
different folder if you want to use it (see below). The configuration
file is simply a set of lines in the following format:
...
\noindent X-RAAS looks for the configuration file in two locations, first
in the \texttt{Output/preferences} folder under X-Plane's global folder and
then in the aircraft folder of the currently loaded aircraft. If both are
present, X-RAAS loads both files in this order, so any parameter set in
the global configuration file in the \texttt{preferences} folder can be
overridden by the configuration file in the aircraft folder, giving the
ability to override global settings on a per-aircraft basis.

\filetext{
\hspace*{-.5em}\fileicon{folder.png} <X-Plane Folder>\\
\hspace*{1.5em}\fileicon{folder.png} Aircraft\\
\hspace*{3em}\fileicon{folder.png} <Your Aircraft's Folder>\\
\hspace*{4.5em}\fileicon{file.png} X-RAAS.cfg\hspace*{2em}{\small
